	function itemsleft(mode=0){
		...
		for(let i=1; i<max_item_ID2; i++){
			if (itemyVShopeA[i][1][3]){
				if (itemyVShopeA[i][1][9] != 11&&itemyVShopeA[i][5]!=99){
					let flag = false;
					for (ii in itemyVShopeA[i][4]){
					if (itemyVShopeA[i][4][ii][1] == 130){
						flag = true;
						totaldaily += itemyVShopeA[i][1][3] + itemyVShopeA[i][1][13];
						}
					}
					if (!flag){
					...
					totalhidden += itemyVShopeA[i][1][3] + itemyVShopeA[i][1][13];
				}
			}
		}
		for(let i in hracVeciA){
			if (hracVeciA[i]){
				if (hracVeciA[i]&&itemyVShopeA[i][1][9] != 11&&itemyVShopeA[i][5]!=99){
					let flag = false;
					for (ii in itemyVShopeA[i][4]){
						if (itemyVShopeA[i][4][ii][1] == 130){
							flag = true;
							havedaily += hracVeciA[i];
						}
					}
					if (!flag){
						have += hracVeciA[i];
		...
		havecur[1] = Math.floor(havecur[1] / 1000);
		totalcur[1] = Math.floor(totalcur[1] / 1000);
		...
		if (mode){
			let itemsdata = Array();
			itemsdata.push([have.toString(), total.toString(), "Stacks obtained(here and below ex. Daily, S&H)"]);
			for (let i in havect){
			...
			itemsdata.push([havedaily.toString(), totaldaily.toString(), "Daily stacks obtained(ex. S&H)"]);
			itemsdata.push([havehidden.toString(), totalhidden.toString(), "Special&Hidden stacks obtained"]);
			return itemsdata;
		}
		else{
			console.log("Stacks obtained(here and below ex. Daily, S&H): " + have.toString() + "/" + total.toString() + " | " +(total - have).toString() + " left.");
			for (let i in havect){
			...
			console.log("Daily stacks obtained: " + havedaily.toString() + "/" + totaldaily.toString() + " | " +(totaldaily - havedaily).toString() + " left.");
			console.log("Special&Hidden stacks obtained: " + havehidden.toString() + "/" + totalhidden.toString() + " | " +(totalhidden - havehidden).toString() + " left.");
			console.log("///");
		}
	}
	...
	}if(PredchadzajuciLevel>410&&(hrac[64]<=0&&hrac[218]*1<=0)&&(PHPtime4%864000>500000)&&_filter==0&&costfilter==0){_subshop=CopyArray(shopItemyFiltered[_filter]);
	_subshop.unshift(itemyVShopeA[3407]);
	}else{if(_filter==0||_filter==9){_subshop=CopyArray(shopItemyFiltered[_filter]);
	for(let i in parsedShopModules){if(parsedShopModules[i]){_subshop.unshift(parsedShopModules[i]);
	}}}else{_subshop=shopItemyFiltered[_filter];
		...
		if(_subshop[i]){
			if(SediCostFilter(_subshop[i])){
				if(pocet_na_stranu<shop_pocet_nastranu_max+1){
					let _out3=ShopItemBlok(_subshop[i]);
					if(_out3!=''){
						...
						//	break;
							...
							if(shop_item_vsetky_pocet>=shop_riadok*shop_stlpcov_nastranu){
								_out2+=_out3;
								pocet_na_stranu++;
								if(pocet_na_stranu%shop_stlpcov_nastranu==0)
									{_out2+='<div class="linedivider"></div>';
								}	
							}
...
function soonItemsBarAdd(_vecID){
	...
	let _lvlrequired = F02h(_vec1);
	let _myMaxLevel=GetMyMaxLevel();
    if(_vec1[5]!=199)if(_vec1[5]!=34&&_vec_cena[9]!=9&&_vec_cena[9]!=10&&_vec_cena[5]<hrac[11]+3)if(_vecID!=3&&_lvlrequired>_myMaxLevel&&_lvlrequired<=_myMaxLevel+50){
		if (itemyVyssiePocet<13){
			itemyVyssiePocet++;
			itemyVyssie.push(' '+ItemImg(_vecID,'posmodFit','class="previewitem '+(_vec_cena[9]==1?'goldenVipBorder':'')+'" '+F02o(LOC_Requiredlevel+': '+_lvlrequired+' ( in '+Bold(_lvlrequired-_myMaxLevel)+' )'+(_vec_cena[5]-hrac[11]>0?'<br>'+LOC_RequiredChapter+': '+(_vec_cena[5]+1)+' ( in '+Bold(_vec_cena[5]-hrac[11])+')':'')+(_vec_cena[9]==1?BR+'VIP item':''))));
			itemyVyssieLvl.push(_lvlrequired);
		}
		for (let i=0; i<itemyVyssiePocet; i++){
			if (_lvlrequired < itemyVyssieLvl[i]){
				for (let i2=itemyVyssiePocet-1; i2>i; i2--){
					itemyVyssie[i2] = itemyVyssie[i2-1];
					itemyVyssieLvl[i2] = itemyVyssieLvl[i2-1];
				}
				itemyVyssie[i] = ' '+ItemImg(_vecID,'posmodFit','class="previewitem '+(_vec_cena[9]==1?'goldenVipBorder':'')+'" '+F02o(LOC_Requiredlevel+': '+_lvlrequired+' ( in '+Bold(_lvlrequired-_myMaxLevel)+' )'+(_vec_cena[5]-hrac[11]>0?'<br>'+LOC_RequiredChapter+': '+(_vec_cena[5]+1)+' ( in '+Bold(_vec_cena[5]-hrac[11])+')':'')+(_vec_cena[9]==1?BR+'VIP item':'')));
				itemyVyssieLvl[i] = _lvlrequired;
				break;
			}
		}
	}
